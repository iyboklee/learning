...
애플리케이션이 데이터베이스에서 값을 읽고, 변경한 값을 다시 쓸때 (read-modify-write 주기라고함) Lost Update가 발생할 수 있다.
...
- 원자적 연산은 해당 Row에 독점적인 잠금을 획득해서 구현하며, 원자적 연산 적용이 가능하다면 적용하는것이 최선의 선택임

원자적 연산이 필요한 기능을 제공하지 않는다면 애플리케이션에서 갱신할 Row에 대해 명시적으로 잠금(FOR UPDATE 구문) 처리할 수 있다.

![EX7_1](assets/ddia/EX7_1.PNG)

read-modify-write 주기가 실행되고 있는 중 다른 트랜잭션이 해당 Row를 읽으려 한다면 먼저 실행된 트랜잭션이 완료될때 까지 대기해야 함
